...
class AgentLLMMixin:
    ...
    def _get_scheduled_context(self) -> str:
        ...
        if not blocks:
            return ""
        # Minimal header — only structural info. All behavioural rules
        # (plan_update usage, MCP constraints, confirmation gates,
        # anti-repeat, etc.) live in the operator's global system prompt
        # / role preset / SOUL — never hardcoded here.
        header = (
            f"\n[Agent workspace: {ws}]\n"
